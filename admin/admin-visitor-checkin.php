...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .page-header::before { content:''; position:absolute; top:-60px; right:-60px; width:220px; height:220px; border-radius:50%; background:rgba(255,255,255,.05); }
        .page-header::after  { content:''; position:absolute; bottom:-80px; right:120px; width:180px; height:180px; border-radius:50%; background:rgba(255,255,255,.03); }
        .page-header-inner { display:flex; align-items:center; justify-content:space-between; position:relative; z-index:1; }
        .page-header h2 { font-family:'Work Sans',sans-serif; font-size:1.8rem; font-weight:800; color:#fff; margin:0 0 4px; letter-spacing:-.02em; display:flex; align-items:center; gap:10px; }
        .page-header .subtitle { color:rgba(255,255,255,.7); font-size:.9rem; margin:0; }
        .admin-badge { display:inline-flex; align-items:center; gap:7px; background:rgba(255,255,255,.15); border:1px solid rgba(255,255,255,.2); border-radius:20px; padding:7px 16px; font-size:.8rem; font-weight:600; color:#fff; backdrop-filter:blur(4px); }

        /* ─── ALERTS ─── */
        .alert { border-radius:var(--radius-sm); border:none; padding:13px 18px; margin-bottom:20px; font-size:.875rem; font-weight:500; display:flex; align-items:center; gap:10px; }
        ...
        /* ─── CARDS ─── */
        .card { background:#fff; border:1px solid var(--border); border-radius:var(--radius); box-shadow:var(--shadow-sm); overflow:hidden; animation:fadeUp .5s ease both; height:100%; }
        .col-lg-5 .card { animation-delay:.25s; }
        .col-lg-7 .card { animation-delay:.30s; }
        .card-header { background:var(--gray-50); border-bottom:1px solid var(--border-lt); padding:16px 20px; display:flex; align-items:center; justify-content:space-between; }
        .card-header-title { font-family:'Work Sans',sans-serif; font-size:.95rem; font-weight:700; color:var(--gray-800); display:flex; align-items:center; gap:9px; margin:0; }
        .card-header-icon { width:30px; height:30px; background:var(--accent-blue-lt); color:var(--accent-blue); border-radius:7px; display:flex; align-items:center; justify-content:center; font-size:13px; }
        .card-header-actions { display:flex; align-items:center; gap:10px; }
        .card-body { padding:20px 22px; }
        .card-body-scroll { padding:20px 22px; max-height:620px; overflow-y:auto; }

        /* ─── SEARCH ─── */
        .search-wrap { position:relative; margin-bottom:16px; }
        .search-wrap .fa-search { position:absolute; left:12px; top:50%; transform:translateY(-50%); color:var(--gray-400); font-size:13px; }
        .search-input { width:100%; padding:9px 12px 9px 34px; border:1px solid var(--border); border-radius:var(--radius-sm); font-family:'DM Sans',sans-serif; font-size:13px; color:var(--gray-800); background:#fff; transition:border-color .2s, box-shadow .2s; outline:none; }
        .search-input:focus { border-color:var(--accent-blue); box-shadow:0 0 0 3px rgba(59,130,246,.12); }
        .search-input::placeholder { color:var(--gray-400); }

        /* ─── REGISTERED VISITOR ITEM ─── */
        .registered-visitor-item:hover { transform:translateY(-1px); box-shadow:var(--shadow-sm); }
        ...
        /* ─── TABLE HEADERS ─── */
        .tbl-head { display:grid; gap:12px; padding:10px 14px; background:var(--gray-50); border:1px solid var(--border-lt); border-radius:var(--radius-sm); margin-bottom:12px; font-size:.7rem; font-weight:700; letter-spacing:.08em; text-transform:uppercase; color:var(--gray-400); }
        ...
        .checkout-row { display:grid; grid-template-columns:1.6fr 1fr 1.2fr 0.7fr 1fr 1fr 1fr; gap:12px; align-items:center; padding:12px 14px; background:#fff; border:1px solid var(--border); border-radius:var(--radius-sm); margin-bottom:6px; transition:transform .15s, box-shadow .15s; }
        .checkout-row:hover { transform:translateY(-1px); box-shadow:var(--shadow-sm); }

        .visitor-row { background:#fff; border:1px solid var(--border); border-radius:var(--radius-sm); margin-bottom:8px; transition:transform .2s, box-shadow .2s; display:grid; grid-template-columns:1.6fr 1fr 1.2fr 0.7fr 1fr 0.75fr 0.9fr; gap:12px; padding:12px 14px; align-items:center; font-size:.85rem; cursor:pointer; }
        .visitor-row:hover { transform:translateY(-1px); box-shadow:var(--shadow-sm); }
        .overdue-row { border-left:3px solid var(--accent-red) !important; background:#fff9f9 !important; }
        .visitor-cell { padding:0; }
        .visitor-name { font-weight:700; color:var(--gray-950); font-size:.9rem; margin-bottom:2px; }
        .visitor-name-information { font-weight:700; color:var(--green-50); font-size: 25px; margin-bottom:2px; }
        .visitor-phone { font-size:.8rem; color:var(--gray-500); }
        .purpose-text { font-size:.82rem; color:var(--gray-700); max-width:200px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
        .group-badge { display:inline-flex; align-items:center; gap:4px; padding:3px 8px; border-radius:12px; font-size:.7rem; font-weight:600; }
        .group-yes { background:var(--green-100); color:var(--green-600); }
        .group-no  { background:var(--gray-100);  color:var(--gray-500); }
        .datetime-text { font-size:.8rem; color:var(--gray-600); font-family:'Space Mono',monospace; }
        .action-buttons { display:flex; gap:6px; align-items:center; }

        /* ─── BUTTONS ─── */
        .btn-refresh { display:inline-flex; align-items:center; gap:6px; padding:7px 14px; background:#fff; border:1px solid var(--border); border-radius:var(--radius-sm); font-family:'DM Sans',sans-serif; font-size:.8rem; font-weight:600; color:var(--gray-600); cursor:pointer; transition:all .2s; }
        .btn-refresh:hover { background:var(--gray-100); color:var(--gray-800); }
        .btn-view { display:inline-flex; align-items:center; justify-content:center; gap:4px; padding:8px 14px; background:linear-gradient(135deg,#3b82f6,#60a5fa); border:none; border-radius:var(--radius-sm); font-family:'DM Sans',sans-serif; font-size:.75rem; font-weight:600; color:#fff; cursor:pointer; transition:all .2s; box-shadow:0 2px 8px rgba(59,130,246,.28); min-width:70px; }
        ...
        .btn-view-outline { display:inline-flex; align-items:center; justify-content:center; gap:4px; padding:6px 12px; background:transparent; border:1px solid var(--accent-blue); border-radius:var(--radius-sm); font-family:'DM Sans',sans-serif; font-size:.75rem; font-weight:600; color:var(--accent-blue); cursor:pointer; transition:all .2s; }
        .btn-view-outline:hover { background:var(--accent-blue-lt); }

        /* ─── STATUS BADGES ─── */
        .badge-status { display:inline-flex; align-items:center; gap:5px; padding:5px 11px; border-radius:14px; font-size:.75rem; font-weight:600; white-space:nowrap; }
        .badge-active { background:var(--green-100); color:var(--green-600); }
        .badge-active .fa-circle { font-size:6px; animation:pulse 2s infinite; }
        @keyframes pulse { 0%,100%{opacity:1} 50%{opacity:.5} }

        /* ─── EMPTY STATE ─── */
        .empty-state { text-align:center; padding:60px 24px; }
        .empty-icon { width:84px; height:84px; margin:0 auto 20px; background:var(--gray-100); color:var(--gray-400); border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:32px; }
        .empty-title { font-family:'Work Sans',sans-serif; font-size:1.15rem; font-weight:700; color:var(--gray-700); margin-bottom:8px; }
        .empty-sub { font-size:.9rem; color:var(--gray-500); max-width:420px; margin:0 auto; }

        /* ─── REFRESH TOAST ─── */
        .refresh-toast { position:fixed; bottom:24px; right:280px; background:var(--accent-blue); color:#fff; padding:8px 18px; border-radius:20px; display:flex; align-items:center; gap:8px; font-size:.8rem; font-weight:600; opacity:0; transform:translateY(12px); transition:all .3s ease; z-index:9999; box-shadow:0 4px 20px rgba(59,130,246,.25); }
        .refresh-toast.show { opacity:1; transform:translateY(0); }

        /* ════════════════════════════════════════════
           VISITOR MODAL — exact match to standalone HTML
        ════════════════════════════════════════════ */
        ...
        .visitor-name { font-size:15px; font-weight:600; color:var(--gray-950); letter-spacing:-.3px; line-height:1.2; }
        ...
        .status-pill.registered { color:#2563eb; background:#eff6ff; border-color:#dbeafe; }
        ...
        .id-meta { font-size:12.5px; }
        .id-meta strong { color:#111827; font-weight:600; display:block; margin-bottom:2px; }
        .id-meta span { color:#9ca3af; }
        ...
        .bl-row { background:#fff; border:1px solid var(--border); border-radius:var(--radius-sm); margin-bottom:8px; display:grid; grid-template-columns:1.4fr 0.9fr 1.6fr 0.75fr 0.7fr 0.85fr 72px; gap:10px; padding:11px 14px; align-items:center; font-size:.84rem; transition:transform .2s, box-shadow .2s; }
        ...
        .sev-badge { display:inline-flex; align-items:center; gap:5px; font-size:.7rem; font-weight:700; letter-spacing:.06em; text-transform:uppercase; padding:3px 9px; border-radius:12px; }
        ...
        .bl-status-badge { display:inline-flex; align-items:center; gap:5px; font-size:.7rem; font-weight:700; padding:3px 9px; border-radius:12px; }
        .bl-active   { background:var(--green-100); color:var(--green-700); }
        .bl-inactive { background:var(--gray-100);  color:var(--gray-500); }

        /* ─── ACTION BUTTONS ─── */
        .btn-blacklist { display:inline-flex; align-items:center; gap:5px; padding:5px 10px; border-radius:6px; background:var(--accent-red-lt); border:1px solid #fecaca; color:var(--accent-red); font-size:.75rem; font-weight:700; cursor:pointer; transition:all .2s; white-space:nowrap; }
        .btn-blacklist:hover { background:var(--accent-red); color:#fff; border-color:var(--accent-red); }
        .btn-deactivate { display:inline-flex; align-items:center; gap:4px; padding:5px 8px; border-radius:6px; background:var(--accent-red-lt); border:1px solid #fecaca; color:var(--accent-red); font-size:.73rem; font-weight:600; cursor:pointer; transition:all .2s; }
        .btn-deactivate:hover { background:var(--accent-red); color:#fff; }
        .btn-activate { display:inline-flex; align-items:center; gap:4px; padding:5px 8px; border-radius:6px; background:var(--green-100); border:1px solid var(--green-200); color:var(--green-700); font-size:.73rem; font-weight:600; cursor:pointer; transition:all .2s; }
        .btn-activate:hover { background:var(--green-500); color:#fff; }

        /* ─── ADD BLACKLIST MODAL ─── */
        .bl-modal-header { background:linear-gradient(135deg,#7f1d1d 0%,#991b1b 50%,#dc2626 100%); padding:22px 26px; border-radius:16px 16px 0 0; position:relative; overflow:hidden; }
        .bl-modal-header h5 { font-family:'Work Sans',sans-serif; font-weight:800; font-size:1.1rem; color:#fff; margin:0; letter-spacing:-.01em; }
        ...
        .form-control-sm-custom { width:100%; padding:8px 12px; border:1px solid var(--border); border-radius:8px; font-family:'DM Sans',sans-serif; font-size:.88rem; color:var(--gray-800); background:#fff; transition:border-color .2s, box-shadow .2s; outline:none; }
